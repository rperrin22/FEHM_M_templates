...
x=min(coor(:,1)):range(coor(:,1))/100:max(coor(:,1));
y=min(coor(:,2)):range(coor(:,2))/100:max(coor(:,2));
[x,y]=meshgrid(x,y);
...
hold on
scatter(coor(:,1),coor(:,2),25,'ks','filled');
for count = 1:length(node)
text(coor(count,1),coor(count,2),sprintf('%d',node(count,1)));
end
